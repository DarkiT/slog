...
func (I *Engine) Mask(inputText string, methodName string) (outputText string, err error) {
	...
	if len(inputText) > DefMaxInput {
		return inputText, fmt.Errorf("DEF_MAX_INPUT: %d , %w", DefMaxInput, errlist.ERR_MAX_INPUT_LIMIT)
	}
...
func (I *Engine) maskStructImpl(inPtr interface{}, level int) (interface{}, error) {
	...
	if val.CanSet() {
		if val.Kind() == reflect.Struct {
			sz := val.NumField()
			if sz > DefMaxInput {
				return inPtr, fmt.Errorf("DEF_MAX_INPUT: %d , %w", DefMaxInput, errlist.ERR_MAX_INPUT_LIMIT)
			}
			for i := 0; i < sz; i++ {
