...
type RuleTestItem struct {
	RuleID int32  `yaml:"RuleID" json:"rule_id"`
	In     string `yaml:"In" json:"in"`
	Out    string `yaml:"Out" json:"out"`
}
type RuleTest struct {
	Date     string         `yaml:"Date" json:"date"`
	TestList []RuleTestItem `yaml:"TestList" json:"test_list"`
}
...
func TestRule(t *testing.T) {
	testPath := "./test/rule_test.json"
	if buf, err := os.ReadFile(testPath); err == nil {
		ruleTestPtr := new(RuleTest)
		if err := json.Unmarshal(buf, ruleTestPtr); err == nil {
