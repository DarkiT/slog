...
type RuleItem struct {
	...
	Detect struct {
		...
		KRelation     string   `yaml:"KRelation"`     // Relation for Key
		VRelation     string   `yaml:"VRelation"`     // Relation for Value
		KAndVRelation string   `yaml:"KAndVRelation"` // Relation for Key And Value
	} `yaml:"Detect"`
...
func NewDlpConfByPath(confPath string) (*DlpConf, error) {
	...
	if fileData, err := os.ReadFile(confPath); err == nil {
		return newDlpConfImpl(fileData)
	} else {
		return nil, err
	}
}
...
func newDlpConfImpl(confString []byte) (*DlpConf, error) {
	if len(confString) == 0 {
